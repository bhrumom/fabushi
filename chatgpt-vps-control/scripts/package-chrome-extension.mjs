...
if (manifest.version !== "0.6.5") throw new Error(`Chrome extension packaging is pinned to 0.6.5; received ${manifest.version}.`);
if (!manifest.permissions?.includes("userScripts")) throw new Error("Fabushi 0.6.5 must expose the integrated userscript runtime.");
if (JSON.stringify(manifest.host_permissions || []) !== JSON.stringify(["<all_urls>"])) {
  throw new Error("Fabushi 0.6.5 must preserve the 0.4.1 all-URL content-script permission used by the existing userscript runner.");
}
