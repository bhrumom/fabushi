...
test("messageRowIds/send regression cannot emit the production-rejected limit=200", () => {
  ...
  assert.match(messageRowIds, /find\(\{ role: "article", limit: 200 \}\)/u);
  ...
  const emitted = serializedFind({ role: "article", limit: 200 });
  assert.deepEqual(emitted, { role: "article", limit: 100 });
});

test("all literal production journey find limits are safe at the serialized controller boundary", () => {
