...
| MSR-107 | Fuse Hermes gateway/transcript semantics into existing Mahayana Rust CLI/runtime | MSR-102 | stdio JSON-RPC + seq/replay + one-turn reasoning/text/tool/result/completion without work-complete card | Mahayana CLI tests + renderer build + protected PR + packaged Desktop E2E | in-progress | exact-head CI, repair, merge, then packaged E2E; broader Hermes parity remains tracked in `09-Hermes能力对齐矩阵.md` |
