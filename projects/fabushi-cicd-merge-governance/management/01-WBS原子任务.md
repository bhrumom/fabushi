...
| FCM-019 | Shopify-style CLI/UI-decoupled fast automation | yes | affected-test selection + real Core test-driver lane + independent Host UI lane + protected merge + exact-main full fallback evidence | in-progress |
| FCM-019.1 | Verify baseline and upstream Shopify fast-CI method | yes | canonical main, current lanes, official affected-test selection method and dependency decision recorded | implemented |
| FCM-019.2 | Add conservative affected Mahayana fast-test selector | yes | modeled paths select stable consumer groups; shared/unknown inputs and main fail safe to full suite; selector tests pass | implemented |
| FCM-019.3 | Preserve Core/UI separation | yes | Core fast gate remains headless; Host UI fast E2E remains independent; packaged/device tests stay post-main | implemented |
| FCM-019.4 | Add first-class developer CLI test entry over the existing Product Core test driver | yes | no-UI local smoke + explicit online probe run through real ProductBackend; release test-driver remains forbidden | pending |
| FCM-019.5 | Prove PR latency reduction on a real leaf change | yes | Actions summary shows reduced selected group surface and retained cache/timing evidence | pending |
| FCM-019.6 | Protected merge and exact-main full-suite validation | yes | required PR checks green, protected merge, exact main readback, full fast suite green, downstream gates non-regressed | pending |
