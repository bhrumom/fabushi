...
| GBF-509 | M5 | Grok study 头像动画行为 parity | GBF-501..504 | 固定 reference SHA；64px 同时间轴；body/eyes/gaze/blink/actions/state transitions 通过预先冻结阈值；禁止复制 proprietary geometry/assets | direct reference-vs-packaged macOS capture + numerical metrics + side-by-side/overlay + video/trace/digest + protected-main rerun | evidence/GBF-509 | IN_PROGRESS | direct v1 motion-parity capture pending | run frozen v1 capture; tune existing FabushiAvatarRuntime only if thresholds fail |
