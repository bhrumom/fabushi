...
## 2026-09-03 — GBF-805 observable product parity continuation
用户要求把当前可观察缺口一次收口：移动 session 重启恢复、桌面显式退出入口、真实 token/delta 流式回复、桌面高耗电、以及 Agent 生成 Mini App 从源码文本升级为可点击运行产物。当前分支 `fix/gbf805-product-parity-20260903` 直接在 canonical Mahayana/runtime 协议上实施：provider 请求开启 SSE 并逐 delta 发出 `ModelEvent::OutputTextDelta`；`TranscriptCard` 新增 `miniApp` 产物并在 desktop/iOS/Android 复用既有 Mini App 容器；桌面头像菜单新增退出登录；移动 session 持久化由 AAC-004 承接；能耗由 MSR-106 承接。任务仍为 IN_PROGRESS，下一门禁是 exact-head Actions compile/unit/E2E，随后 protected main、exact-main packaged visual/mobile-relaunch evidence 与 Release。

### 2026-09-03 — GBF-805 real macOS test-build review

- Tested `1.2.14-test.152` on the installed target Mac rather than relying on the old packaged E2E assumptions.
- Product review is blocked by the test artifact's ad-hoc signing/keychain boundary: SecurityAgent asks for the login-keychain password for `mahayana-app-host` / `com.ombhrum.fabushi.auth.v2`, followed by Host request timeouts.
- Agent send, logout/re-auth, and formal E2E recalibration are therefore not accepted yet. The `fabushi test` connector also fails its account connection with HTTP 400.
- Evidence: `evidence/GBF-805/mac-test-1.2.14-test.152-product-review-20260903.md`.

### 2026-09-03 — GBF-410 public MCP connector recovered

- Root cause for the `fabushi test` account-connection failure was a full public MCP host filesystem: read-only health/OAuth metadata remained up, but DCR POST returned 502.
- Reclaimed only archived journals, package cache, stale Fabushi temp data, Node compile cache, Docker build cache and unused images; persistent MCP state, active containers and volumes were preserved.
- Host now has about 2.2 GiB free; DCR returns 201; `fabushi test` resolves `fabushi_mcp_ci_test` and `list_devices` executes successfully.
- Same-account Mac device discovery/control is the remaining live acceptance after the new signed Mac test artifact is installed.
