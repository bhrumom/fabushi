...
## 2026-09-05 — M3-DESKTOP-003 test-driven acceptance failure / architecture replan

- Re-read canonical `main` as `586a0952f17ab4b36dab9a69402b837968f5aa3f`; task authority remains `FAB-P0001 / TFI`. MSR (`FAB-P0005`) and GBF (`FAB-P0004`) were read as cross-project boundaries and remain non-target/read-only.
- Re-read product PR #2349 exact head `ec2ca86e7873b340115d3acc69b8b1d2dacda2f0`, base architecture SHA `8fb9c16493f6b78a466356137820b57f200f4ed0`, test failure PR #2350 head `20e0b5b38a97215cc3beb3bed1f67332e075aeee`, and binding comment `5551040795`.
- Exact-head Electron run `33959034172` is success across Linux/macOS/Windows. Successful packaged returning-user metrics are hundreds of milliseconds and do not reproduce the reported ~1 minute delay; `rootCauseClaim` stays `null`.
- Raw Linux job `101287508113` proves the first packaged target attempt failed at `seededConversationId == ""`; retry #1 passed. This is bounded as a scenario seed/setup nondeterminism, not a product startup root cause.
- The target accepted artifact set also lacks a dedicated returning-user `.webm` and explicit app/main-process logs. Grok visual-evidence videos are not acceptable substitutes.
- Split the unresolved parent into two serial minimal tasks: `M3-DESKTOP-003-SEED-001` first, then `M3-DESKTOP-003-EVIDENCE-001`. Both implementation allowlists are exactly `desktop/e2e/messenger.spec.ts` plus task-specific TFI records/evidence.
- Existing `desktop/playwright.config.ts` already enables trace/video and outputs `test-results`; existing diagnostics upload already covers `desktop/test-results/**`. Existing `grok-visual-evidence.spec.ts` proves in-test `page.screencast.start/stop` is available. Therefore no workflow, Grok test, product source or native change is authorized for evidence capture.
- `M3-DESKTOP-004` remains blocked. Architecture will not authorize a product performance fix until parent 003 returns with deterministic, complete exact-scenario evidence and a measured bottleneck.
- This architecture round modifies only `projects/telegram-fabushi-integration/**`; it does not implement code, review, merge, test release or stable release.