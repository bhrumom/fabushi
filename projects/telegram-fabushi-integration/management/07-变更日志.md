...
## 2026-09-06 — TFI-M9-GLOBAL-DHARMA-IOS-001 implementation start

- Frozen canonical main `8f7e83902a616ecdb62fdaded65ea79227e745f3` and created governed branch `feat/tfi-ios-global-dharma-commerce-20260906`.
- Added open-source-first StoreKit/Advanced Commerce design; retained Rust Fabushi Pay + PLATFORM_DB as the only payment/entitlement authority.
- Added `feature.marketplace.add` through Mahayana Product/Host so iOS local install also creates canonical account Mini App/Bot state.
- Added Global Dharma Bot `打开应用` semantic/visible action backed by the same installed Mini App Host/runtime.
- Added native account-scoped CNY 1080 lifetime commerce model, Advanced Commerce signed request path, StoreKit transaction/server verification, explicit restore and canonical entitlement gate.
- Added six-tool semantic elements for entitlement/buy/restore and shell return navigation.
- Added Swift contract tests for exact product/SKU/price/rail and Advanced Commerce envelope; no local iOS build/test executed.
- Recorded Apple external gates and StoreKit Testing exclusion; payment remains blocked until real sandbox prerequisites exist.

## 2026-09-06 — iOS Global Dharma #2446 rebaseline

- Rebased by controlled merge onto canonical `main@8595a50196309c8ebb91c3f8077125d7dc9e3ffa` (`9d51e3e3d514cfcc2b6337e1baaecdc51c8453d3`), preserving branch history.
- Applied the exact GitHub Actions rustfmt delta in Mahayana test-driver `backend.rs`.
- Split iOS commerce verification into bounded CI Simulator canonical-ledger/no-charge mode and production Advanced Commerce/StoreKit mode; both remain server-entitlement-gated.
- No local `xcodebuild` or Simulator test was run.

## 2026-09-06 — test-driver external artifact parity

- Recorded Marketplace Contract `34048201512` live failure and diagnostics artifact `9993892782`.
- Added test-driver native→`mobile` artifact fallback to match the existing production App Host while preserving native request/device/account platform identity.
- Added a focused unit test for fallback mapping; no local build or `xcodebuild` was run.

## 2026-09-06 — correct native-platform versus universal-artifact test contract

- Recorded Marketplace Contract `34048908027` and diagnostics artifact `9994122473`.
- Corrected live test to assert native request/device `platform=ios` at the response boundary and universal release artifact support `mobile`.
- Kept `InstalledPluginPointer` schema unchanged; no fake `platform` field was added to persisted receipt. No local build or `xcodebuild` was run.

## 2026-09-06 — align live test with canonical InstalledPluginPointer

- Recorded Marketplace Contract `34049664397` and diagnostic artifact `9994229393`.
- Replaced stale persisted receipt assertions `sha256/sourceUrl` with canonical `artifactId/artifactSha256/version/runtime` across install, post-install list and restart persistence checks.
- Release manifest continues to prove selected artifact ID/runtime/SHA/platform; marketplace source metadata continues to prove external provenance. No product receipt schema or runtime behavior changed; no local build or `xcodebuild` was run.
...
## 2026-09-06 — re-align iOS Global Dharma to #2445 shared-runtime main

- Re-read live main as `c82b29cd6404c2f19b93d8479b2e2cae45469249` after #2445 merged; #2446 no longer treats `8595a501…` as current canonical main.
- Merged the new shared WebMCP/runtime/account/commerce authority baseline into the iOS branch, preserving all TFI/AAC evidence from both lines.
- Reallocated the iOS AAC cross-project task to `AAC-005` / `AAC-R015` because canonical #2445 owns `AAC-004` / `AAC-R011..R014`.
- Updated M9.T14 to distinguish CI canonical-ledger/no-charge acceptance from the separately blocked production Apple Advanced Commerce sandbox rail.

## 2026-09-06 — apply exact rustfmt after c82b alignment

- Recorded iOS test-driver `34050259724` / artifact `9994335885`.
- Applied only the formatter-emitted multiline shape for the persisted receipt version/runtime assertions. No local Rust build or `xcodebuild` was run.
...
## 2026-09-07 — align #2446 to e926e02e and close two current-head fast failures

- Merged canonical `main@e926e02e94fc2408738ce7ec678f2267baa4a0e1` into #2446 as `395fc59b5de3941892e2c3e45757987e58e84633`, preserving both TFI/AAC governance histories.
- Removed the obsolete Marketplace test assertion for absent `marketplaceSource.marketplaceHostsPackage`; retained `sourceRef`, `repository`, release-manifest and selected-artifact provenance checks.
- Normalized `management/wbs/M9.md` to one newline at EOF after Native mobile `34050415248` identified the extra blank.
- Recorded Marketplace diagnostic artifact `9994523246`. No local build/test performed; new Actions and exact-main packaged evidence remain authoritative.

## 2026-09-07 — follow canonical main through #2455

- Re-read live main and found #2455 already protected-merged as `8adfa009f2fdf349e9f0c659fcfa4176ff7d7c2c`.
- Merged that baseline into #2446 without conflicts; new baseline adds only desktop Mini App parity E2E coverage.
- Invalidated `e980cf93…` as final merge evidence and require a fresh exact-head CI set before queue entry.

## 2026-09-07 — align Marketplace provenance assertion with canonical source_json

- Recorded exact-head Marketplace failure `34053892448` / `101542174216` and diagnostic `9995497157`.
- Replaced stale `sourceRef` assertion with `provider=fabushi-official`, GitHub repository URL and 40-hex commit checks; retained external-release and selected-artifact integrity assertions.
- No runtime code or commerce/auth semantics changed; no local build/test performed.

## 2026-09-07 — apply Actions-emitted provenance rustfmt delta

- Recorded Mahayana fast `34054520721` / job `101543811294` formatting-only failure.
- Applied exactly the rustfmt expansion emitted by Actions for the source-commit assertion. No semantic/runtime change and no local build/test.
...
## 2026-09-07 — rebase acceptance on installed Mini App route repair

- Recorded fully green predecessor Marketplace run `34054593453` / job `101544034838` and artifacts `9995817557`, `9995765469`.
- Re-read protected main before queue entry and detected #2459 route repair at `43ce998fd5fbcae032c179a8814de9ec08d03f4c`; fail-closed instead of merging against stale `8adfa009…`.
- Merged new main into #2446 as `4fd797fe76353ea9876c89d1a3d9641d4e65bed1`, preserving both TFI histories. No local build/test performed.
