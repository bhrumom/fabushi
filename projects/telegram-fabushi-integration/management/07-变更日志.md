...
## 2026-08-23 — MiniApp service-call execution boundary changed to MCP-only

- 用户明确修正 M8-CALL-001：AI、语音、DTMF、聊天输入只是解释/路由入口，MiniApp 实际业务功能统一通过目标 MiniApp 的 MCP 接口执行。
- 新增不变量：MiniApp 没有对应 MCP Tool 时，该功能不可用；禁止 AI 使用通用 Agent、宿主 native capability 或其它平行 action executor 补做。
- DTMF 和聊天纯数字统一为确定性 `digits -> exposed MCP Tool + arguments -> tools/call`；无 route 返回 unavailable。
- final speech / chat natural language 改为 `ResolveMcpTool`，resolver 仅获得目标 MiniApp 的真实 Tool catalog；catalog 外 Tool 由 Rust domain 二次拒绝。
- `MiniAppServiceActionRequest/Result` 与 `ResolveIntent` 早期抽象被替换为 `MiniAppMcpToolCapability`、`MiniAppMcpResolveRequest`、`MiniAppMcpToolResolution`、`MiniAppMcpInvocationRequest/Result`、`InvokeMcpTool`。
- 现有 `frontend/packages/mcp-app-sdk` / Web MiniApp Host 的 `tools/list`、`tools/call`、Tool annotations、approval/sensitive-input 被确认为后续 Host 接线的 canonical runtime；禁止另造第二套 MiniApp 业务 RPC。
- M8 WBS 新增/细化：M8-CALL-002 streaming STT + Conversation projection；M8-CALL-003 real `tools/list` catalog + constrained MCP resolver + existing `tools/call` executor；M8-CALL-004 composer routing。
- PR #2063 标题与描述同步改为 MCP-driven；latest-head GitHub Actions、protected merge、canonical-main readback 前任务仍为 `IN_PROGRESS`。
