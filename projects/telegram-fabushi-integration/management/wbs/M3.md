...
### 2026-09-07 M3.T03 exact-main generation-freshness update

- `main@71168adbeea65e998bb650ba3a4636911287636a` Electron run `34058850412` formal macOS package failed the semantic message-context action because an intentionally truncated 500-element snapshot could not lease the off-snapshot message target and its generation advanced before mutation.
- **状态**：`IN_PROGRESS` / product stale-generation contract remains fail-closed.
- **原子修复**：fresh exact `find(agentId)` immediately before the off-snapshot mutation; bounded retry only on `stale_app_surface_generation`; no product security relaxation.
- **证据**：macOS diagnostics artifact `9996959351`; task `TFI-MACOS-SEMANTIC-MESSAGE-ACTIONS-001`.
- **下一步**：required PR CI → merge queue → newer canonical-main three-platform Electron gate + post-main evidence.
