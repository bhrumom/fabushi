...
## 2026-09-05 — M3-DESKTOP-003 test-driven acceptance blocker replan — authoritative latest

- canonical `main` re-read: `586a0952f17ab4b36dab9a69402b837968f5aa3f`; M3 task architecture base remains `8fb9c16493f6b78a466356137820b57f200f4ed0`; product PR #2349 exact head is `ec2ca86e7873b340115d3acc69b8b1d2dacda2f0` and remains OPEN.
- exact-head Electron desktop quality run `33959034172` / #1673 is SUCCESS across Linux/macOS/Windows. Successful packaged returning-user results are Linux ~302.3ms renderer→list / 408.8ms renderer→composer / 647ms launch→list; macOS ~342.1 / 1396.9 / 631; Windows ~458.9 / 563.3 / 737. The reported ~1 minute delay was not reproduced and `rootCauseClaim:null` remains binding.
- Linux raw job `101287508113` first packaged target attempt failed because `seededConversationId` was empty after create/select; retry #1 passed. Therefore aggregate green does not prove deterministic scenario setup.
- Exact target evidence is incomplete: no dedicated target `.webm` and no explicit app/main-process logs. Separate GBF/Grok visual-evidence video is not valid substitution.
- test group records-only failure PR #2350 head `20e0b5b38a97215cc3beb3bed1f67332e075aeee` and #2349 comment `5551040795` are accepted as `TEST-FAIL / BLOCKED` inputs.
- architecture split the unresolved parent into `M3-DESKTOP-003-SEED-001` (`FROZEN / NEXT-ONLY-EXECUTABLE`) and `M3-DESKTOP-003-EVIDENCE-001` (`FROZEN / BLOCKED-BY-SEED-001`). Both permit only `desktop/e2e/messenger.spec.ts` for implementation plus task-specific TFI records/evidence.
- `M3-DESKTOP-004` remains `BLOCKED`; no startup product fix, review, protected merge, test release or stable release is authorized from current evidence.
- MSR and GBF remain read-only non-target project boundaries. Any need to alter product source, Host/auth/native/protocol/schema, workflow, retry policy, dependency, version/release, MiniApp/MSR/GBF behavior is scope expansion and must return to Architecture.