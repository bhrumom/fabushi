...
- **整体阶段**：M3 桌面聊天完整交互 — `IN_PROGRESS`；M7 unified Messenger remediation 并行 `TESTING`；M8 MiniApp MCP service-call contract 并行 `IN_PROGRESS`。
- **M0**：`TESTED`。
...
- **当前 M8 service-call 任务**：`M8-CALL-001`，分支 `feat/tfi-miniapp-ai-service-calls`，PR #2063。
...
## 2026-08-23 — M8-CALL-001 MiniApp MCP 客服电话契约修正

- 用户明确补充：MiniApp 电话/聊天中的业务功能全部由目标 MiniApp 的 MCP Tool 驱动；AI、DTMF、聊天输入不能直接执行业务，也不能在 MiniApp 无对应功能时由通用 Agent/宿主补做。
- 原始补充已经持久化到 `source/2026-08-23-miniapp-ai-service-calls.md`；ADR-0008、专题规范、WBS、验收矩阵和 Evidence 同步更新。
- Rust `miniapp_service_call.rs` 已从早期 `ResolveIntent/ActionResult` 抽象重构为 MCP-only：
  - `MiniAppMcpToolCapability` 对齐现有 MCP Apps `tools/list` catalog；
  - DTMF 与聊天纯数字共用 `MiniAppMcpDtmfRoute`；
  - final speech/chat natural language 生成 `ResolveMcpTool`；
  - catalog 外 Tool 选择返回 `McpToolUnavailable`；
  - 无匹配 Tool / 无 DTMF route 返回 `McpCapabilityUnavailable`；
  - 真实业务执行 effect 只有 `InvokeMcpTool`；
  - `MiniAppMcpInvocationResult` 作为可审计 MCP 结果。
- 已确认现有 `frontend/packages/mcp-app-sdk` 与 Web MiniApp Host 已具备 `tools/list`、`tools/call`、Tool annotations、approval/sensitive-input 基础；后续 M8-CALL-003 必须复用，不新增第二套业务 RPC。
- PR #2063 已更新为 `feat: add MCP-driven MiniApp AI service call core`；latest-head GitHub Actions 已触发。current-head Rustfmt/unit/Clippy/required gates、protected merge、canonical-main readback 完成前，M8-CALL-001 维持 `IN_PROGRESS`。
- 下一原子任务：M8-CALL-002 streaming STT + Conversation projection；M8-CALL-003 真实 `tools/list` snapshot + constrained MCP resolver + `tools/call` Host executor；M8-CALL-004 composer 数字/自然语言 routing。
