...
## 2026-09-06 — TFI-M9-GLOBAL-DHARMA-IOS-001

Canonical baseline frozen at `8f7e83902a616ecdb62fdaded65ea79227e745f3`; governed branch `feat/tfi-ios-global-dharma-commerce-20260906` created. iOS gaps were reproduced from source: local Marketplace install did not account-sync the Mini App/Bot; Global Dharma Bot header had a non-interactive desktop icon; `FabushiPayStoreKit` had no product caller; Apple provider is Advanced Commerce and its server verification requires a pre-signed request. The branch now adds Host-level account install, Bot `打开应用`, native Global Dharma commerce state, Advanced Commerce JWS StoreKit purchase, explicit restore and semantic controls. No local `xcodebuild` has been run. Protected PR CI and exact-main packaged evidence are pending.

Payment remains fail-closed until external Apple facts are real: Advanced Commerce/Mini Apps Partner access, generic product/provider binding active, explicit tax code, Apple credentials and sandbox account/device. Apple documents that Advanced Commerce cannot use Xcode StoreKit Testing, so a local `.storekit` pass will not be counted as the requested ¥1080 payment evidence.

## 2026-09-06 — TFI-M9-GLOBAL-DHARMA-IOS-001 canonical-main alignment / ledger test-mode

- #2446 final pre-alignment head `c0c050b6120e744849a71d1df3fe05ec372413b6` was re-read against canonical `main@8595a50196309c8ebb91c3f8077125d7dc9e3ffa`.
- Controlled merge completed without conflicts as `9d51e3e3d514cfcc2b6337e1baaecdc51c8453d3`.
- Four failed checks (`34039955034`, `34039955089`, `34039955156`, `34039955007`) share one root cause: the same `backend.rs` rustfmt delta; substantive protocol/marketplace tests had not yet executed.
- iOS Simulator acceptance is now explicitly test-mode canonical-ledger-only: CNY 1080 purchase and restore must not invoke StoreKit or create a real charge and must unlock only after canonical server entitlement re-read. Production iOS retains Advanced Commerce/StoreKit and its external Apple blockers.
- State remains `IN_PROGRESS / PR CI PENDING`; exact-main packaged video/screenshots/`.xcresult`/trace/report remain `PENDING`.

## 2026-09-06 — iOS Marketplace external-artifact parity follow-up

- PR head `228f33315e3d1979d8aaae1f6dc60b728522aa84` cleared rustfmt, Mahayana fast, iOS test-driver online search and Global Dharma Rust gates.
- Marketplace Contract `34048201512` then exposed a new live-only parity defect: official `global-dharma@1.0.0` is a universal `mobile` local-web artifact, while the test-driver directly requested `ios`; diagnostic artifact `9993892782` (`sha256:457de7cee04532c9fc561244f70989f5fc9245da45401f69e69842650802d52b`).
- Production App Host already has native `ios|android` → `mobile` artifact fallback. The follow-up changes only the test-driver to mirror that behavior and does not alter iOS device/account semantics.
- State remains `IN_PROGRESS / PR CI PENDING`; no merge or packaged/video acceptance is claimed.

## 2026-09-06 — iOS Marketplace receipt-contract follow-up

- Marketplace Contract `34048908027` confirmed the external-package fallback itself succeeded; `native_marketplace_install_mirrors_app_host_mobile_artifact_fallback` passed and the previous incompatible-artifact error did not recur.
- The next failure is test-only: live assertion expected `receipt.platform == ios`, but the canonical `InstalledPluginPointer` does not persist platform. Native request/device identity is already preserved at response top-level `platform=ios`; the official universal artifact correctly uses `mobile`.
- Diagnostic artifact `9994122473`, digest `sha256:2a9d801c476cf5fcc6dae44c373cc13ec8b2dbf62cbecf7a639a3f4b96adbc8d`.
- Follow-up changes only the stale assertions; product runtime, account/session boundary and installer receipt schema remain unchanged. State remains `IN_PROGRESS / PR CI PENDING`; packaged video remains `PENDING`.

## 2026-09-06 — iOS Marketplace persisted-receipt contract follow-up

- Marketplace Contract `34049664397` advanced beyond the native/mobile fixes and failed only because the live test still read legacy receipt keys `sha256/sourceUrl`.
- Canonical `InstalledPluginPointer` is camelCase and persists `pluginId`, `version`, `artifactId`, `artifactSha256`, `runtime`, `entry`, `requestedPermissions`, `installedPath`; it intentionally does not persist source URL or platform.
- Diagnostic artifact `9994229393`, digest `sha256:24ef6fb9b2fb76c6c1f739e12d07cda92ec02994adecec61b8ff58ebcf28a6d8`.
- Follow-up updates all install/list/restart assertions in the live test to the canonical receipt schema and keeps external provenance assertions on `marketplaceSource` + release manifest. Product runtime/account/session/receipt schema are unchanged. State remains `IN_PROGRESS / PR CI PENDING`; complete packaged video remains `PENDING`.
...
## 2026-09-06 — #2446 re-align to canonical main c82b29cd / AAC-005

- While #2446 was resolving Marketplace live-contract failures, canonical main advanced from `8595a50196309c8ebb91c3f8077125d7dc9e3ffa` to `c82b29cd6404c2f19b93d8479b2e2cae45469249` through #2445.
- #2445 is a direct dependency, not unrelated drift: it establishes the canonical Global Dharma shared WebMCP Bot/Web runtime, delegated session-bound Mini App identity and server commerce/entitlement authority.
- #2446 therefore re-aligns to `c82b29cd…` before new CI. Canonical AAC `AAC-004` remains the controlled account-session task from #2445; this iOS commerce/account boundary is reallocated to `AAC-005` with acceptance row `AAC-R015`.
- No iOS product code conflict occurred; merge conflicts were governance-only and resolved by preserving both evidence chains. Exact-main packaged video remains `PENDING`.

## 2026-09-06 — post-c82b rustfmt-only follow-up

- Exact-head iOS test-driver `34050259724` on `36024463982d8c45604fd61275e9f2f1c1e099d9` failed only at formatting; protocol/live steps were skipped.
- Artifact `9994335885` (`sha256:44c4f870d2b7ae20c353f315dc0b68384cde58faa00677688b25c6b5a2a84d72`) was uploaded by the fail-closed path.
- The correction is the exact Actions rustfmt output for two long `assert_eq!` calls. AAC-004/AAC-005 account semantics and iOS commerce behavior are unchanged; PR remains `IN_PROGRESS`, packaged video `PENDING`.
...
## 2026-09-07 — iOS #2446 re-aligned to e926e02e

- Canonical main is now `e926e02e94fc2408738ce7ec678f2267baa4a0e1`; #2446 alignment merge is `395fc59b5de3941892e2c3e45757987e58e84633` with no product-code conflicts.
- Marketplace predecessor `34050415486` / `101532905722` failed only at the stale `marketplaceHostsPackage=false` assertion after all earlier universal-artifact and canonical-receipt checks had advanced; diagnostic `9994523246` (`sha256:2b6f4c987de8506f80abd744822a44ae2b1596bb8c54bac5f7a2e74195de6b57`).
- Native mobile predecessor `34050415248` / `101532904942` failed only on an extra EOF blank in M9 WBS. Both are corrected without changing runtime semantics. New GitHub Actions must pass before protected merge.
- Exact-main packaged iOS journey and complete video remain `PENDING`; CI Simulator commerce remains canonical-ledger/no-charge only.

## 2026-09-07 — #2446 follows main@8adfa009

- Canonical main advanced to `8adfa009f2fdf349e9f0c659fcfa4176ff7d7c2c` via #2455 while #2446 CI was in progress. The iOS branch merged that exact main with no conflicts; only the desktop Mini App parity E2E changed from the new baseline.
- All earlier `e980cf93…` results are non-final after this drift. New-head Actions are required before protected merge. Packaged iOS/video evidence remains `PENDING`.

## 2026-09-07 — Marketplace canonical source shape follow-up

- `34053892448` / `101542174216` failed only on obsolete `marketplaceSource.sourceRef`; install/receipt/release-manifest checks had already passed. Canonical official source is `provider=fabushi-official`, GitHub repository URL and commit SHA.
- Diagnostic `9995497157` (`sha256:3e7ea09afb9c521764f21f8f39695e47d087bbb62515eff7af5a63935f938348`). Branch test now checks the actual canonical source contract instead of `sourceRef`. Exact-main package/video remains `PENDING`.
...
## 2026-09-07 — iOS follows canonical Marketplace route repair

- #2446 old head `7f951162…` achieved full PR validation, including fully green Marketplace cloud E2E `34054593453` / `101544034838` and required CI `34054593269`.
- Main then advanced to `43ce998fd5fbcae032c179a8814de9ec08d03f4c` via #2459, which is directly relevant: it adds the canonical account-installed Mini App command route that Android packaged evidence had shown missing.
- Alignment merge `4fd797fe76353ea9876c89d1a3d9641d4e65bed1` is complete; old green results are non-final after drift. New exact-head CI and then exact-main packaged iOS evidence are required. Video remains `PENDING`.
