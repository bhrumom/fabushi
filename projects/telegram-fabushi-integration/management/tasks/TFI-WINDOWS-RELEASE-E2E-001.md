...
## 2026-09-06 Windows 1.2.49 governed retest after iOS AX repair

PR #2434 protected-merged the remote-computer Accessibility containment repair as `a2c25b12f64e3e8076bd456f90bd5d49a1677154`. The failed `1.2.48` candidate is superseded. From that exact canonical main, the Windows release train advances strictly to global version `1.2.49`; full acceptance still requires same-SHA Electron + Native iOS/Android success, successful post-main source binding/publication, a fresh run-bound App-owned Windows device, all six semantic App tools plus CI-session controls, the complete Windows functional journey, final `settings-logout`, and complete always-upload evidence.
