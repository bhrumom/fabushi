...
PR #2434 protected-merged the remote-computer Accessibility containment repair as `a2c25b12f64e3e8076bd456f90bd5d49a1677154`. The failed `1.2.48` candidate is superseded. While the Windows release PR was validating, protected Android PR #2436 advanced canonical main to global version `1.2.49` as `496d9f0910b1fced2eb594eded5310d5b8e5612c`. The existing Windows PR was therefore dequeued, updated with that exact latest main, and advanced strictly again to global version `1.2.50`; full acceptance still requires same-SHA Electron + Native iOS/Android success, successful post-main source binding/publication, a fresh run-bound App-owned Windows device, all six semantic App tools plus CI-session controls, the complete Windows functional journey, final `settings-logout`, and complete always-upload evidence.
