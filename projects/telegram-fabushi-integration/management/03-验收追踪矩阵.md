...
## M3-DESKTOP-003 diagnostic reacceptance — 2026-09-05

| Requirement | Atomic task | State | Test-driven acceptance |
|---|---|---|---|
| deterministic real returning-user setup | `M3-DESKTOP-003-SEED-001` | `FROZEN / NEXT-ONLY-EXECUTABLE` | exact final head; packaged target reaches non-empty created conversation identity on original attempt (`retry=0`); real create/select path; durable 32-message seed + persistence condition preserved; no product semantic/retry-policy change |
| exact target evidence completeness | `M3-DESKTOP-003-EVIDENCE-001` | `FROZEN / BLOCKED-BY-SEED-001` | after SEED TEST-PASS, same original-attempt packaged target on Linux/macOS/Windows emits dedicated target `.webm`, app/main process logs, timing JSON, P0–P9 JSON, screenshot(s), trace and exact-head/platform/task binding; no Grok-video substitution |
| diagnostic root-cause boundary | parent `M3-DESKTOP-003` | `TEST-FAIL / BLOCKED` | current successful run proves only non-reproduction at ~302–459ms renderer→list and preserves `rootCauseClaim:null`; no product root-cause claim until complete deterministic evidence supports one |
| measured product fix | `M3-DESKTOP-004` | `BLOCKED` | may be unfrozen only by Architecture after parent 003 returns with a measured bottleneck; green aggregate run alone is insufficient |

Binding failure evidence: PR #2349 exact head `ec2ca86e7873b340115d3acc69b8b1d2dacda2f0`; workflow_dispatch Electron run `33959034172` three-platform success; Linux job `101287508113` first packaged target attempt failed because `seededConversationId == ""`, retry passed; accepted target artifact set lacked dedicated target video and app/main logs; records-only failure PR #2350 head `20e0b5b38a97215cc3beb3bed1f67332e075aeee`; comment `5551040795` = `TEST-FAIL / BLOCKED`.

Neither retry success nor another scenario's visual evidence may satisfy these rows. MSR/GBF are read-only non-target projects for this replan.