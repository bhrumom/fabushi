...
|---|---|---|
| RespondCommunityJoin compile/event boundary | `engine.rs` + focused approve/reject test; implementation `30c6104e...` | PENDING CI |
| Community create/update authority | service idempotent create no-op + forged kind/owner/participant regression | PENDING CI |
| Missing Community regression | `RequestCommunityJoin -> CommunityNotFound` focused test | PENDING CI |
| Non-Community create compatibility | focused ordinary Direct create test | PENDING CI |
| Independent real-diff review | PR #2323 | PENDING |
| Protected merge + exact-main installable/package E2E + Release | no evidence yet | BLOCKED/PENDING |

Required runs `33884739411` and `33884739286` failed at inherited rustfmt before Rust compile/test; no pass is claimed. See `evidence/TFI-M6-P0-001/`.

- TFI-M6-P0-001 task-specific compile/Unit/Contract/Security checks: PASS on run `33885842476` / job `101065268536`. Required-CI acceptance: FAIL/BLOCKED on `33885842392` / `33885842303` due inherited rustfmt; no overall acceptance pass.
