...
| M9-IOS-GD-R06 CNY 1080 lifetime purchase | server offer → intent → Advanced Commerce JWS → StoreKit → Pay verify → entitlement | contract tests + real Apple sandbox | implemented / sandbox blocked pending external config |
| M9-IOS-GD-R07 restore + permission gate | explicit `AppStore.sync` + verified transaction + Pay reverify + canonical `access.allowed` | contract tests + real Apple sandbox restore | implemented / sandbox blocked pending external config |
