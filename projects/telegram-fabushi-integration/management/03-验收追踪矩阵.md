...
## Active M9 iOS Global Dharma acceptance — 2026-09-06

| Requirement | Implementation | Verification | Status |
|---|---|---|---|
| M9-IOS-GD-R01 Marketplace account install | `feature.plugin.install` + new Host `feature.marketplace.add` | Rust/iOS CI + packaged semantic search/install | implemented / CI pending |
| M9-IOS-GD-R02 Bot appears from canonical account sync | require `accountSynchronized` + canonical Bot id; existing `bot.list` | packaged Messages snapshot/find/assert | implemented / exact-main pending |
| M9-IOS-GD-R03 natural-language WebMCP | existing `chat.send(mode: agent)` + unified Host/runtime | semantic Bot send + streamed `agent.step` + Mini App state | existing path / exact-main pending |
| M9-IOS-GD-R04 Telegram-like open app | `mobile-bot-open-app` → installed `MiniAppWebMcpSurface` | action/snapshot/assert on App-owned iOS device | implemented / CI pending |
| M9-IOS-GD-R05 automatic Fabushi login | native `feature.auth.deviceAgentSession`; no WebView credential exposure | packaged test-account session, no second login | implemented / exact-main pending |
| M9-IOS-GD-R06 CNY 1080 lifetime purchase | CI Simulator: canonical ledger/no-charge; production: server offer → intent → Advanced Commerce JWS → StoreKit → Pay verify → entitlement | contract tests + CI packaged ledger journey; production Apple sandbox tracked separately | implemented / CI ledger pending; production sandbox externally blocked |
| M9-IOS-GD-R07 restore + permission gate | CI Simulator: canonical `/v1/purchases/restore`; production: `AppStore.sync` + verified transaction + Pay reverify; both require canonical `access.allowed` | contract tests + CI packaged restore; production Apple sandbox tracked separately | implemented / CI restore pending; production sandbox externally blocked |
| M9-IOS-GD-R08 evidence | exact-main package/video/checkpoint screenshots/xcresult/trace/logs | GitHub Actions artifacts | pending |
...
## 2026-09-07 — #2446 e926e02e re-alignment evidence gate

| Gate | Evidence | Status |
|---|---|---|
| Canonical alignment | merge `395fc59b5de3941892e2c3e45757987e58e84633`, parents `caeba222…` + `e926e02e…` | IMPLEMENTED / new CI pending |
| Marketplace live package contract | predecessor `34050415486` / job `101532905722` failed only on removed `marketplaceHostsPackage` test field; diagnostic `9994523246` | FIXED IN BRANCH / re-run pending |
| Native PR hygiene | predecessor `34050415248` / job `101532904942` failed only on extra EOF blank in M9 WBS | FIXED IN BRANCH / re-run pending |
| iOS test-driver | predecessor `34050415217` | SUCCESS on previous aligned head; re-run required after e926e02e alignment |
| Exact-main packaged Global Dharma journey | install → Bot → WebMCP → open Web UI → durable revision sync → auto-login → canonical-ledger CNY1080 purchase/restore | PENDING |
| Complete video/screenshots/`.xcresult`/trace/report/logs | exact accepted main only | PENDING |
...
## 2026-09-07 — iOS #2446 route-repair baseline gate

| Gate | Evidence | Status |
|---|---|---|
| Previous exact-head full PR validation | `7f951162…`: Marketplace `34054593453`, iOS driver `34054593251`, Native `34054593248`, fast `34054593237`, Global Dharma Rust `34054593261`, required CI `34054593269` | PASS on superseded `main@8adfa009…`; historical only |
| Canonical route-repair alignment | merge `4fd797fe76353ea9876c89d1a3d9641d4e65bed1`, parents `7f951162…` + `43ce998f…` | IMPLEMENTED / new CI required |
| Bot natural-language installed Mini App routing | canonical `/v1/marketplace/plugins/:plugin_id/route` from #2459 | INCLUDED IN NEW BASELINE / packaged proof pending |
| Exact-main iOS packaged journey | search/install → Bot → NL WebMCP → open app → durable revision sync → auto-login → canonical-ledger CNY1080 buy/restore → entitlement | PENDING |
| Complete video/screenshots/`.xcresult`/trace/report/logs | exact accepted main only | PENDING |
