...
## 2026-09-03 round 7 — AAC-004 mobile session durability
用户再次复现移动端完全退出应用后重新打开需要重新登录。新任务 AAC-004 将根因从 UI 状态收敛到 Rust 加密 session 的移动端密钥生命周期：生产 iOS/Android 现在由 Keychain/Android Keystore 生成并恢复稳定的设备级 256-bit 存储密钥，再只通过 native Host 创建边界注入 Rust；token/passphrase 不进入 UserDefaults、明文 SharedPreferences、renderer 或模型。新增 Rust manager-recreation regression 与 CI source gate；任务保持 in-progress，直到 iOS/Android Actions 编译、完整进程重启登录保持、显式 logout 后重启仍未登录、protected main 和 Release 证据齐全。
