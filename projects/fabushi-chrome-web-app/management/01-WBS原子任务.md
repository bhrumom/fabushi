...
| CWA-012 | Make installed Marketplace userscripts update from their own Tampermonkey-style `@updateURL` | CWA-005 + userscript metadata contract | CWA-R023; stale catalog does not block URL-based metadata/source update, identity and HTTPS checks | in-progress / Chrome `0.6.13` released and local profile updated; canonical host PR pending | Protected host PR, packaged extension evidence, and live update readback |
