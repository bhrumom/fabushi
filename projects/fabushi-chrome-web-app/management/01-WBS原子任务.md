...
| CWA-012 | Make installed Marketplace userscripts update from their own Tampermonkey-style `@updateURL` | CWA-005 + userscript metadata contract | CWA-R023; stale catalog does not block URL-based metadata/source update, identity and HTTPS checks | in-progress / canonical `0.6.13` package passed; Web Store publish blocked by existing review | Web Store review, required packaged evidence, and live update readback |