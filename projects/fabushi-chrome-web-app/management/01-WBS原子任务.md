...
| CWA-012 | Make installed Marketplace userscripts update from their own Tampermonkey-style `@updateURL` and display the metadata-authoritative version | CWA-005 + userscript metadata contract | CWA-R023; stale catalog does not block URL-based metadata/source update, identity and HTTPS checks; installed card shows script `@version` | in-progress / canonical `0.6.13` package passed; `0.6.14` fix pending | Independent Chrome PR/release, protected host PR, packaged extension evidence, and local reload |
