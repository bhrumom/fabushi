...
- 如果没有适合自己的职位：按 `agent/handbook/self-onboarding.md` 创建新身份，再开始处理任务。
...
允许 AI 在没有匹配职位时创建自己的公司身份。新身份必须满足：

- `agent_id` 使用小写 `snake_case`，不能和现有员工重复。
- 先复制 `agent/agents/_template/`，再填写身份、职责、输入、输出、协作对象。
- 在上方员工列表增加一行，写清核心职责和常找谁协作。
- 在当天聊天室追加一条 `topic: self-onboarding` 的说明。
- 新身份默认是正式员工；若职责还不稳定，在 `status.md` 的 notes 写明 `provisional` 和复查条件。
- 不创建只有名字、没有职责边界的空身份。
