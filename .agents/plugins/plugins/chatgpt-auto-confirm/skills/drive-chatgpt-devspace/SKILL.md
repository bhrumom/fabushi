...
If Actions, deployments, releases, or remote checks are running and require a long time to complete, do not run `sleep` commands to poll inside the chat. The chat session may time out. Instead, generate a handoff report with `status` set to `incomplete`, populate `wait_seconds` with an estimated wait time (e.g. 300), describe the wait in `wait_reason`, and include a concrete `next_task` for the next Chat to resume checking.

For GitHub-backed work, use the GitHub connector for cloud state and bhrum2 for the local checkout. Set `next_connector` when the next fresh Chat must switch between those contexts. Treat disconnects, DNS failures, upstream 502/503/504, and connector timeouts as a recoverable wait, and use `wait_seconds` to pause before repeating a failed request.
