...
// .wrangler/tmp/bundle-id0uiV/checked-fetch.js
...
function getAlipayEnvConfig(env) {
  ...
  return {
    ...
    isSandbox,
    gateway: isSandbox ? "https://openapi-sandbox.dl.alipaydev.com/gateway.do" : ALIPAY_CONFIG.GATEWAY_URL
  };
}
...
async function handleCreateAlipayOrder(request, env) {
  try {
    ...
    const subject = `\u529F\u5FB7\u65E0\u91CF\u5168\u7403\u53D1\u9001 - ${planDetails.name}`;
    const orderData = {
      orderId: outTradeNo,
    ...
    const params = {
      ...
      notify_url: new URL("/api/alipay/notify", request.url).href,
...
async function handleAlipayNotify(request, env) {
  try {
    ...
    if (params.trade_status === "TRADE_SUCCESS" || params.trade_status === "TRADE_FINISHED") {
      ...
      const startDate = currentMembership.isActive ? new Date(currentMembership.endDate) : /* @__PURE__ */ new Date();
      const endDate = new Date(startDate.getTime() + planDetails.duration);
      user.membershipType = "paid";
      user.membershipEndDate = endDate.toISOString();
      await env.USERS_KV.put(`user:${user.username}`, JSON.stringify(user));
      console.log(`Membership activated for user ${user.username} until ${user.membershipEndDate}`);
    }
    return new Response("success", { status: 200 });
  } catch (error) {
...
// .wrangler/tmp/bundle-id0uiV/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-id0uiV/middleware-loader.entry.ts
