...
function webMcpBootstrap(pluginId: string, nonce: string): string {
  return `<script>(function(){
    ...
    function ensureStateBar(){let bar=document.getElementById('fabushi-host-state');if(bar)return bar;bar=document.createElement('div');bar.id='fabushi-host-state';bar.setAttribute('data-testid','fabushi-miniapp-host-state');bar.style.cssText='position:fixed;left:8px;right:8px;bottom:8px;z-index:2147483647;padding:8px 10px;border-radius:10px;background:rgba(10,18,30,.92);color:#eef6ff;font:12px/1.35 system-ui;box-shadow:0 4px 24px rgba(0,0,0,.3);pointer-events:none';document.body.appendChild(bar);return bar;}
    function render(state){if(!state||!document.body)return;currentExecution=state;const bar=ensureStateBar();bar.dataset.revision=String(state.revision??0);bar.dataset.phase=String(state.phase||'idle');bar.dataset.tool=String(state.tool||'');bar.dataset.surface=String(state.surface||'home');bar.dataset.entitlementAllowed=String(state.entitlementAllowed);bar.textContent='Fabushi · r'+bar.dataset.revision+' · '+bar.dataset.phase+' · '+bar.dataset.surface+(bar.dataset.tool?' · '+bar.dataset.tool:'')+(state.progress?' · '+state.progress:'');window.dispatchEvent(new CustomEvent('fabushi:execution-state',{detail:state}));}
