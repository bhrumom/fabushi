...
fn humans_and_bots_share_the_same_conversation_engine() {
    ...
    engine.execute(Command::UpsertActor { actor: Actor::human("human:1", "善友") }).unwrap();
    engine.execute(Command::UpsertActor { actor: Actor::assistant("agent:mahayana", "大乘助手") }).unwrap();
    engine.execute(Command::UpsertActor { actor: Actor::bot("bot:publisher", "发布机器人") }).unwrap();
    ...
    engine.execute(Command::UpsertConversation { conversation }).unwrap();
    let events = engine.execute(Command::QueueMessage {
        ...
        content: MessageContent::Text { text: FormattedText::plain("请两个机器人协作处理") },
    ...
    assert_eq!(engine.state().messages[&ConversationId::new("chat:unified")].len(), 1);
}

#[test]
fn invoice_and_paid_order_use_the_same_chat_domain() {
    let mut engine = MessagingEngine::new();
    engine.execute(Command::UpsertActor { actor: Actor::human("buyer", "购买者") }).unwrap();
    engine.execute(Command::UpsertActor { actor: Actor::bot("seller", "商店机器人") }).unwrap();
    engine.execute(Command::UpsertConversation {
        conversation: Conversation::direct(
            "chat:shop",
            "商店",
            vec![participant("buyer", ParticipantRole::Owner), participant("seller", ParticipantRole::Member)],
            1,
        ),
    }).unwrap();
    let invoice = Invoice {
        ...
        currency: "USD".into(),
        prices: vec![PriceLine { label: "Pro".into(), amount: Money::new("USD", 999) }],
    ...
    engine.execute(Command::UpsertOrder {
        order: PaymentOrder {
            id: "order:1".into(),
...
fn mini_app_permissions_are_enforced_before_host_calls() {
    ...
    engine.execute(Command::InstallMiniApp { manifest }).unwrap();
    engine.execute(Command::GrantMiniApp {
        grant: MiniAppGrant {
    ...
    engine.execute(Command::OpenMiniApp {
    ...
    let error = engine.execute(Command::MiniAppCall {
        session_id: "session:1".into(),
        request_id: "request:1".into(),
        request: MiniAppRequest::RequestLocation,
    }).unwrap_err();
    assert_eq!(error, EngineError::MiniAppPermissionDenied(MiniAppPermission::Location));
}
...
fn wire_protocol_is_fabushi_owned_and_versioned() {
    ...
    assert_eq!(envelope.protocol_version, FABUSHI_MESSAGING_PROTOCOL_VERSION);
}
