...
static UDP_SOCKETS: Lazy<Mutex<HashMap<String, UdpSocket>>> = Lazy::new(|| Mutex::new(HashMap::new()));
...
pub extern "C" fn fabushi_runtime_http_fetch_json(request_json: *const c_char) -> *mut c_char {
    ffi_result(|| http_fetch_json(read_json_request(request_json)?))
}

#[no_mangle]
pub extern "C" fn fabushi_runtime_udp_open_json(request_json: *const c_char) -> *mut c_char {
    ffi_result(|| udp_open_json(read_json_request(request_json)?))
}

#[no_mangle]
pub extern "C" fn fabushi_runtime_udp_send_json(request_json: *const c_char) -> *mut c_char {
    ffi_result(|| udp_send_json(read_json_request(request_json)?))
}

#[no_mangle]
pub extern "C" fn fabushi_runtime_udp_broadcast_json(request_json: *const c_char) -> *mut c_char {
    ffi_result(|| udp_broadcast_json(read_json_request(request_json)?))
}

#[no_mangle]
pub extern "C" fn fabushi_runtime_udp_close_json(request_json: *const c_char) -> *mut c_char {
    ffi_result(|| udp_close_json(read_json_request(request_json)?))
}

fn read_json_request(ptr: *const c_char) -> Result<Value, RuntimeError> {
    if ptr.is_null() {
        return Err(RuntimeError::new("invalid_request", "request pointer is null"));
    }
...
fn ffi_result(mut run: impl FnMut() -> Result<Value, RuntimeError>) -> *mut c_char {
    let value = match run() {
...
fn json_to_c_string(value: Value) -> *mut c_char {
    ...
    CString::new(text)
        .unwrap_or_else(|_| CString::new("{\"ok\":false,\"errorCode\":\"internal_error\",\"message\":\"invalid response string\"}").unwrap())
        .into_raw()
}
...
fn http_fetch_json(params: Value) -> Result<Value, RuntimeError> {
    ...
    let max_body_bytes = read_u64(&params, "maxBodyBytes", 2 * 1024 * 1024)
...
fn udp_open_json(params: Value) -> Result<Value, RuntimeError> {
    ...
    let broadcast = params.get("broadcast").and_then(Value::as_bool).unwrap_or(false);
    socket
...
fn udp_send_json(params: Value) -> Result<Value, RuntimeError> {
    ...
    let sent_bytes = {
        ...
        let socket = sockets
            .get(&socket_id)
            .ok_or_else(|| RuntimeError::new("socket_not_found", format!("UDP socket not found: {socket_id}")))?;
        socket
...
fn udp_broadcast_json(params: Value) -> Result<Value, RuntimeError> {
    let host = params
        .get("host")
        .and_then(Value::as_str)
        .map(str::trim)
    ...
    let sent_bytes = if let Some(id) = socket_id.as_deref() {
        ...
        let socket = sockets
            .get(id)
            .ok_or_else(|| RuntimeError::new("socket_not_found", format!("UDP socket not found: {id}")))?;
        socket
            .set_broadcast(true)
            .map_err(|error| RuntimeError::new("udp_broadcast_failed", error.to_string()))?;
        socket
...
fn udp_close_json(params: Value) -> Result<Value, RuntimeError> {
    let socket_id = required_string(&params, "socketId")?;
...
fn request_body(params: &Value) -> Result<Option<Vec<u8>>, RuntimeError> {
    if let Some(body) = params.get("body") {
        return Ok(Some(body.as_str().unwrap_or(&body.to_string()).as_bytes().to_vec()));
    }
