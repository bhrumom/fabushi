...
// .wrangler/tmp/bundle-6PwoyQ/checked-fetch.js
...
// .wrangler/tmp/bundle-6PwoyQ/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-6PwoyQ/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
__name(getTextFiles, "getTextFiles");
...
__name(getAllTextFilesFromManifest, "getAllTextFilesFromManifest");
...
async function handleSearch(request, env) {
  try {
    ...
    console.log("Search query:", query2, "includeAll:", includeAll);
    if (!query2) {
    ...
    for (const file of files) {
      ...
      if (titleLower.includes(queryLower) || contentLower.includes(queryLower)) {
        ...
        if (index !== -1) {
          ...
          const end = Math.min(content.length, index + query2.length + 150);
...
async function handleToggleLike(request, env, db) {
  try {
    ...
    if (action === "like") {
      ...
      ).bind(contentId, contentType, userId, (/* @__PURE__ */ new Date()).toISOString()).run();
    } else if (action === "unlike") {
      if (userId) {
        await db.prepare("DELETE FROM content_likes WHERE content_id = ? AND user_id = ?").bind(contentId, userId).run();
      } else {
        await db.prepare("DELETE FROM content_likes WHERE content_id = ? AND user_id IS NULL").bind(contentId).run();
      }
    }
