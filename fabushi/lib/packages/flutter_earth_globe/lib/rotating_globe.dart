...
class RotatingGlobeState extends State<RotatingGlobe>
  ...
  Future<SphereImage?> buildSphere(double maxWidth, double maxHeight) async {
    ...
    final spherePixels = _cachedPixels!;
    // clear buffer
    spherePixels.fillRange(0, totalPixels, 0);
    ...
    // Cache matrix storage to avoid calling methods inside the hot loop.
    // Matrix3 is column-major in vector_math_64 (m00=0, m10=1, m20=2, m01=3, m11=4, m21=5, m02=6, m12=7, m22=8)
    final rotX = rotationMatrixX.storage;
    final rotZ = rotationMatrixZ.storage;
    ...
    for (var y = minY; y < maxY; y++) {
      ...
      for (var x = minX; x < maxX; x++) {
        ...
        if (zSquared > 0) {
          ...
          double z3 = rotZ[2] * x2 + rotZ[5] * y2 + rotZ[8] * z2;

          final lat = math.asin(z3 / sphereRadius);
          final lon = math.atan2(y3, x3);
