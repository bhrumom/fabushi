...
class _VideoFeedViewTextContentState extends State<VideoFeedViewTextContent>
  ...
  void initState() {
    super.initState();
    _ownerId =
        'text_content_${hashCode}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  void _setupListeners() {
    ...
    if (_visibilityListener == null) {
      try {
        ...
        _visibilityListener = () =>
            _onPageVisibilityChanged(visibilityNotifier.isVideoFeedVisible);
  ...
  void _removeListeners() {
    ...
    if (_visibilityListener != null) {
      try {
        context.read<VideoFeedVisibilityNotifier>().removeListener(
          _visibilityListener!,
        );
      } catch (e) {
        // context已不可用
  ...
  void _onPageVisibilityChanged(bool isPageVisible) {
    ...
    _debugLog(
      '📱 TTS: Page visibility changed to ${isPageVisible ? "VISIBLE" : "HIDDEN"}',
    );
  ...
  void _onHighlightAnimationTick() {
    ...
    final calibratedProgress = (rawProgress + _calibrationOffset).clamp(
      0.0,
      1.0,
    );
    final targetIndex = (calibratedProgress * _words.length).floor().clamp(
      0,
      _words.length - 1,
    );
  ...
  void _parseContent() {
    ...
    if (isLargeText) {
      ...
      _debugLog(
        'TTS MV: Large text detected (${widget.textContent.length} chars), showing preview first',
      );
    }
  ...
  Future<void> _initTts() async {
    ...
    try {
      ...
      _debugLog(
        '📱 TTS TextContent: TTS ready | speechRate=${_ttsManager.speechRate} | '
      ...
      _currentMsPerChar = _baseMsPerChar;
      _debugLog(
        '📱 TTS TextContent: Init error: $e, using default msPerChar=$_baseMsPerChar',
      );
    }
  }
    ...
    _ttsManager.registerCallbacks(
      ...
      onProgress: (text, start, end, word) {
        ...
        if (_progressCallbackCount == 1) {
          ...
          _debugLog(
            '📱 TTS ✅ Progress callback working, calibration mode active',
          );
        }
        ...
        if (_ttsManager.isMacOS && _useSentenceMode && _playing) {
          ...
          if (end >= currentText.length - 1) {
            _debugLog(
              '📱 TTS (Mac): Progress near end (end=$end, len=${currentText.length})',
            );
          }
        }
      },
      onCompletion: () {
        ...
        _debugLog(
          '📱 TTS 🔔 Completion callback received | '
        ...
        Future.microtask(() {
          ...
          if (_currentSentenceId != capturedSentenceId) {
            _debugLog(
              '📱 TTS ⚠️ Stale sentence completion callback (expected=$_currentSentenceId, got=$capturedSentenceId), ignoring',
            );
            return;
          }
          ...
          if (_ttsManager.isMacOS && elapsed < 100) {
            _debugLog(
              '📱 TTS ⚠️ Mac: Suspiciously fast completion (${elapsed}ms), likely from stop(), ignoring',
            );
            return;
          }
  ...
  void _calibrateFromProgress(int end) {
    ...
    if (_progressCallbackCount % 5 == 0) {
      _debugLog(
        '📱 TTS 🎯 Calibration: error=${error.toStringAsFixed(3)}, offset=${_calibrationOffset.toStringAsFixed(3)}',
      );
    }
  }
  ...
  void _onSentenceComplete() {
    ...
    if (nextSentence < _sentences.length) {
      final totalPause =
          _sentencePauseMs + (_ttsManager.isMacOS ? _macExtraDelayMs : 0);
      _debugLog(
        '📱 TTS ⏸️ Pause ${totalPause}ms before sentence $nextSentence',
      );
  ...
  Future<void> _playSentence(int sentenceIndex) async {
    ...
    final expectedDuration = _calculateExpectedSentenceDuration(
      _words.length,
    ).round();
    ...
    _debugLog(
      '📱 TTS 📖 New sentenceId=$sentenceId for sentence $sentenceIndex',
    );

    final sentence = _sentences[sentenceIndex];
    _debugLog(
      '📱 TTS 📖 Playing sentence $sentenceIndex: "${sentence.substring(0, sentence.length.clamp(0, 20))}..."',
    );
  ...
  void _startHighlightAnimation() {
    ...
    _debugLog(
      '📱 TTS 🎬 Started highlight animation: ${duration}ms for ${_words.length} chars',
    );
  }
  ...
  void _stopPlayback() {
    ...
    _debugLog(
      '📱 TTS TextContent: Stop triggered - stack: ${StackTrace.current.toString().split('\n').take(5).join(' | ')}',
    );
    _playing = false;
    _waitingForCompletion = false;
    _stopHighlightAnimation();
  ...
  void didUpdateWidget(VideoFeedViewTextContent old) {
    ...
    if (old.isVisible != widget.isVisible) {
      _debugLog(
        '📱 TTS TextContent: Visibility ${old.isVisible} -> ${widget.isVisible} for $_ownerId',
      );
      ...
      Future.microtask(() {
        ...
        if (widget.isVisible && !_playing) {
          ...
          _debugLog(
            '📱 TTS TextContent: Becoming invisible, stopping playback',
          );
          _stopPlayback();
        }
      });
    ...
    if (old.textContent != widget.textContent) {
      _debugLog(
        'TTS MV: Content actually changed, invalidating session $_playbackSessionId',
      );
      _debugLog(
        'TTS MV: Old content length=${old.textContent.length}, New=${widget.textContent.length}',
      );
      ...
      child: Container(
        ...
        child: Center(
          child: Padding(
            ...
            child: RepaintBoundary(child: _buildOptimizedKaraokeText()),
          ),
        ),
      ),
    );
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      shadows: [
                        ...
                        const Shadow(color: Colors.yellow, blurRadius: 30),
                      ],
                    ),
                  ),
                ),
