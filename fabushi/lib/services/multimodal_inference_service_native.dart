...
class MultimodalInferenceService {
  ...
  Future<void> initialize(
    ...
    try {
      ...
      // 配置上下文参数（使用级联操作符设置属性）
      final contextParams = ContextParams()..nCtx = nCtx;
  ...
  Future<String> describeImage(
    ...
    try {
      // 发送带图像的提示并等待完成
      final result = await _llamaParent!.sendPromptWithImages(effectivePrompt, [
        image,
      ]);
      return result ?? '';
    } finally {
  ...
  Future<String> chat(
    ...
    debugPrint(
      '  文本: ${textPrompt.substring(0, textPrompt.length.clamp(0, 50))}...',
    );
    ...
    try {
      ...
      if (hasImages) {
        // 创建 LlamaImage 列表
        final llamaImages = images!
            .map((bytes) => LlamaImage.fromBytes(bytes))
            .toList();

        // 发送带图像的提示
        result = await _llamaParent!.sendPromptWithImages(
          textPrompt,
          llamaImages,
        );
      } else {
