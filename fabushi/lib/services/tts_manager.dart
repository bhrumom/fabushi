...
class TtsManager {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _tts!.setCompletionHandler(() {
        ...
        if (_isStopping) {
          debugPrint(
            '📱 TTS Manager: Ignoring completion callback triggered by stop()',
          );
          _isStopping = false;
          return;
        }
      ...
      _isInitialized = true;
      debugPrint(
        '📱 TTS Manager: Initialized | Device=$_deviceBrand | FallbackOnly=$_useFallbackOnly',
      );
    } catch (e) {
  ...
  Future<void> _detectDevice() async {
    try {
      if (Platform.isAndroid) {
        ...
        _useFallbackOnly = problematicBrands.any(
          (brand) => _deviceBrand.contains(brand),
        );
        debugPrint(
          '📱 TTS Manager: Device brand="$_deviceBrand", useFallbackOnly=$_useFallbackOnly',
        );
  ...
  void registerCallbacks({
    ...
    if (_activeOwnerId != null && _activeOwnerId != ownerId) {
      debugPrint(
        '📱 TTS Manager: Owner $ownerId taking over from $_activeOwnerId',
      );
    }
  ...
  Future<bool> speak(String text, String ownerId) async {
    ...
    if (_isSpeaking) {
      debugPrint(
        '📱 TTS Manager: Stopping previous speech before starting new one',
      );
      await stop();
      // 等待TTS引擎释放资源，避免 -8 错误
    ...
    try {
      debugPrint(
        '📱 TTS Manager: Starting speech for owner $ownerId (${text.length} chars)',
      );
      _isSpeaking = true;
