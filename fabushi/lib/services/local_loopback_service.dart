...
class LocalLoopbackService {
  ...
  LocalLoopbackService({this.onLog, this.onHeartbeat});
    ...
    if (_cachedData != null) {
      ...
      if (await file.exists()) {
        ...
        while (offset < fileData.length) {
          final end = (offset + _maxChunkSize < fileData.length)
              ? offset + _maxChunkSize
              : fileData.length;
          final packetSize = header.length + (end - offset);
          final packet = Uint8List(packetSize);
  ...
  Future<void> _startMainThreadMode() async {
    ...
    try {
      _mainThreadSocket = await RawDatagramSocket.bind(
      ...
      _mainThreadTimer = Timer.periodic(Duration(milliseconds: intervalMs), (
        timer,
      ) {
  ...
  static void _isolateEntry(_IsolateParams params) async {
    ...
    try {
      ...
      sendPort.send(
        '🔥 [Worker] 开始极速回环: ${prebuiltPackets.length} 包/轮 x $batchCount 批',
      );
