...
class MeditationSessionManager extends ChangeNotifier {
  static final MeditationSessionManager _instance =
  ...
  String get currentSutra =>
      _currentSutra ?? _lockedPractice?.title ?? _lastSutra ?? '默认功课';
  ...
  bool get isPracticeLocked =>
      _lockedPractice != null && _lockedPractice!.isLocked;
  ...
  double get progressPercent {
    ...
    return (_currentDuration.inSeconds / (_preferredDurationMinutes * 60))
        .clamp(0.0, 1.0);
  }
  ...
  Future<void> loadPreferences() async {
    ...
    try {
      ...
      _preferredDurationMinutes =
          prefs.getInt(_storageKey('zen_preferred_duration')) ?? 30;

      // 加载锁定的功课
      final practiceJson = prefs.getString(_storageKey('zen_locked_practice'));
      if (practiceJson != null) {
        try {
          _lockedPractice = MeditationPractice.fromJson(
            jsonDecode(practiceJson),
          );
      ...
      debugPrint(
        '🧘 禅室偏好加载: 用户=$_currentUsername, 上次功课=$_lastSutra, 锁定功课=${_lockedPractice?.title}, 建议时长=$_preferredDurationMinutes分钟',
      );
      notifyListeners();
    } catch (e) {
  ...
  Future<void> _savePreferences() async {
    try {
      ...
      await prefs.setInt(
        _storageKey('zen_preferred_duration'),
        _preferredDurationMinutes,
      );
      ...
      if (_lockedPractice != null) {
        await prefs.setString(
          _storageKey('zen_locked_practice'),
          jsonEncode(_lockedPractice!.toJson()),
        );
      }
    ...
    try {
      ...
      if (cloudPractice is Map && cloudPractice['title'] != null) {
        ...
        _lockedPractice = MeditationPractice(
          title: title,
          filePath:
              (cloudPractice['filePath'] ??
                      'manual:${Uri.encodeComponent(title)}')
                  .toString(),
          selectedAt:
              DateTime.tryParse(
                (cloudPractice['selectedAt'] ?? '').toString(),
              ) ??
              DateTime.now(),
          isLocked: true,
        );
  ...
  Future<void> _syncLockedPracticeToCloud() async {
    ...
    try {
      final response = await HttpService.post(
        ...
        body: {'mainPractice': _lockedPractice!.toJson()},
        useAuth: true,
      );
  ...
  Future<SessionResult> endSession() async {
    if (!_isInSession) {
      return SessionResult(success: false, message: '没有进行中的修行');
    }
    ...
    debugPrint(
      '🧘 修行结束: ${result.sutra}, 时长${result.duration.inMinutes}分钟, 念诵${result.chantCount}遍',
    );
    notifyListeners();
