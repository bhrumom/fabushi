...
class BertTokenizer {
  ...
  static Future<BertTokenizer> fromAsset(String path, {bool doLowerCase = true}) async {
  ...
  List<int> encode(String text, {int maxLen = 512}) {
    ...
    final truncatedIds = ids.length > maxLen - 2 ? ids.sublist(0, maxLen - 2) : ids;
    ...
    for (final char in cleaned.runes) {
      ...
      if (_isCJK(char)) {
        if (buffer.isNotEmpty) {
            tokens.addAll(buffer.toString().split(RegExp(r'\s+')).where((t) => t.isNotEmpty));
            buffer.clear();
        }
        tokens.add(s);
