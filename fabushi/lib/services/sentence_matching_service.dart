...
class SentenceMatchingService {
  ...
  Future<void> loadConfig() async {
    ...
    _configLoaded = true;
    debugPrint(
      '[SentenceMatching] 配置已加载: 快速切换=${(_fastMatchThreshold * 100).toInt()}%, 普通匹配=${(_matchThreshold * 100).toInt()}%',
    );
  }
  ...
  double calculateMatchScore(String recognizedText, String targetSentence) {
    ...
    final lcsLength = _longestCommonSubsequenceLength(
      recognizedPinyin,
      targetPinyin,
    );
  ...
  bool shouldAdvanceToNext(
    ...
    debugPrint(
      '[SentenceMatching] 匹配度: ${(score * 100).toStringAsFixed(1)}%, '
