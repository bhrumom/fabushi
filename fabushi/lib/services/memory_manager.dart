...
class MemoryManager {
  ...
  Future<void> trimCacheIfNeeded() async {
    ...
    if (totalCacheMB > maxCacheMemoryMB) {
      debugPrint(
        '🧹 [MemoryManager] 缓存超出限制 (${totalCacheMB.toStringAsFixed(1)}MB > ${maxCacheMemoryMB}MB)，开始清理...',
      );
      ...
      for (final cache in sortedCaches) {
        ...
        debugPrint(
          '  🗑️ [MemoryManager] ${cache.cacheName}: ${(before / 1024).toStringAsFixed(0)}KB -> ${(after / 1024).toStringAsFixed(0)}KB',
        );
      }
    }
  }
