...
class LlamaInferenceService {
  ...
  static LlamaInferenceService get instance =>
      _instance ??= LlamaInferenceService._();
  ...
  Future<void> initialize(String modelPath, {int? nCtx}) async {
    ...
    if (Platform.isMacOS && Llama.libraryPath == null) {
      ...
      final macOSDir = executablePath.substring(
        0,
  ...
  Stream<String> generateStream(String prompt) {
    ...
    void checkCompletion(CompletionEvent event) {
      if (promptId != null && event.promptId == promptId) {
        debugPrint(
          'LlamaInferenceService: 推理完成 (promptId=$promptId, success=${event.success}, error=${event.errorDetails})',
        );
    ...
    sub = _llamaParent!.stream.listen(
      (token) {
        ...
        if (tokenCount <= 3 || tokenCount % 50 == 0) {
          debugPrint(
            'LlamaInferenceService: 收到token #$tokenCount: ${token.length > 20 ? token.substring(0, 20) + "..." : token}',
          );
        }
    ...
    completionSub = _llamaParent!.completions.listen((event) {
      debugPrint(
        'LlamaInferenceService: 收到 completion event (eventPromptId=${event.promptId}, myPromptId=$promptId, success=${event.success})',
      );
      if (promptId != null) {
