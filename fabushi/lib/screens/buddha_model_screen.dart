  ...
  void _updateSmoke(double dt) {
    ...
    for (int i = 0; i < _particleCount; i++) {
      ...
      if (p.y > tipPos.y + 45.0 || p.y < -500) {
        p.x = tipPos.x + (_random.nextDouble() - 0.5) * 0.5;
        p.y = tipPos.y + (_random.nextDouble() * 2.0);
        p.z = tipPos.z + (_random.nextDouble() - 0.5) * 0.5;
      } else {
        final speed = _particleSpeeds[i];
        p.y += speed * 30.0 * dt;
        final heightFactor = ((p.y - tipPos.y) / 45.0).clamp(0.0, 1.0);
        p.x +=
            windX * heightFactor * 1.5 + math.sin(time * 2.0 + p.y * 0.5) * 0.1;
        p.z +=
            windZ * heightFactor * 1.5 + math.cos(time * 1.5 + p.y * 0.5) * 0.1;
      }
    }
  }
