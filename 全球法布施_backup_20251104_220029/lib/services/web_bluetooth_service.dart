...
class WebBluetoothService {
  ...
  Future<bool> connectGatt() async {
    ...
    try {
      final gattServer = await js_util.promiseToFuture(js_util.callMethod(_device, 'gatt.connect', []));
      final service = await js_util.promiseToFuture(js_util.callMethod(gattServer, 'getPrimaryService', [SERVICE_UUID]));
      _characteristic = await js_util.promiseToFuture(js_util.callMethod(service, 'getCharacteristic', [CHARACTERISTIC_UUID]));
      ...
      js_util.callMethod(_characteristic, 'addEventListener', ['characteristicvaluechanged', onCharacteristicValueChanged]);
  ...
  Future<void> _sendMetadata(String fileName, int fileSize) async {
    ...
    try {
      ...
      await js_util.promiseToFuture(js_util.callMethod(_characteristic, 'writeValue', [metaDataBytes]));
  ...
  Future<void> _sendFileContent(Uint8List fileBytes, String fileName, int fileSize) async {
    ...
    for (var i = 0; i < fileBytes.length; i += chunkSize) {
      ...
      try {
        await js_util.promiseToFuture(js_util.callMethod(_characteristic, 'writeValue', [fullPacket]));
        sentChunks++;
  ...
  Future<void> _sendEndMarker(String fileName) async {
    ...
    try {
      ...
      await js_util.promiseToFuture(js_util.callMethod(_characteristic, 'writeValue', [endMarkerBytes]));
