...
async function main() {
  ...
  if (scaffoldIfMissing && !(await deployEnvExists(deployEnvPath))) {
    console.log("== Scaffold deploy env ==");
    const scaffoldArgs = [
      "--preset",
      args["scaffold-preset"]?.trim() || "read-only-preview",
      "--deploy-env-path",
      deployEnvPath,
      "--forum-image",
      args["forum-image"]?.trim() || "ghcr.io/bhrumom/fabushi-forum:main",
      "--forum-port",
      args["forum-port"]?.trim() || "3000",
      "--forum-data-dir",
      args["forum-data-dir"]?.trim() || "./data",
      "--deploy-check-url",
      normalizeUrl(args["deploy-check-url"]?.trim() || ""),
      "--data-source",
      args["data-source"]?.trim() || "sqlite",
      "--writes-enabled",
      args["writes-enabled"] ?? "",
      "--write-access-code",
      args["write-access-code"] ?? "",
      "--deployment-stage",
      args["deployment-stage"] ?? "",
      "--public-base-url",
      normalizeUrl(args["public-base-url"]?.trim() || ""),
    ];

    await runNodeScript("scaffold-deploy-env.mjs", scaffoldArgs);
    console.log("");
  }
