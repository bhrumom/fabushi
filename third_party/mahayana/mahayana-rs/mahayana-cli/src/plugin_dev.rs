...
pub fn github_source_identity(plugin_path: &Path) -> Result<Value, String> {
    ...
    let repository = env::var("MAHAYANA_SOURCE_REPOSITORY")
    ...
    let repository_id = env::var("MAHAYANA_SOURCE_REPOSITORY_ID")
        .or_else(|_| env::var("GITHUB_REPOSITORY_ID"))
        .ok()
    ...
    let commit = match env::var("MAHAYANA_SOURCE_COMMIT").or_else(|_| env::var("GITHUB_SHA")) {
