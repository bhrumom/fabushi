...
-- New sessions start at the minimum useful privilege already supported by Fabushi today:
-- display + input. Clipboard, files and audio remain opt-in and disabled until their
-- explicit approval APIs/data planes are implemented.
...
BEGIN
    INSERT INTO remote_computer_session_grants
        (session_id, user_id, device_id, client_id,
         allow_display, allow_input, allow_clipboard, allow_file_transfer, allow_audio,
         granted_at, expires_at, revoked_at)
    VALUES
        (NEW.session_id, NEW.user_id, NEW.device_id, NEW.client_id,
         1, 1, 0, 0, 0, NEW.created_at, NEW.expires_at, NULL);

    INSERT INTO remote_computer_audit_events
        (user_id, device_id, client_id, session_id, event_type, provider, route, created_at)
    VALUES
        (NEW.user_id, NEW.device_id, NEW.client_id, NEW.session_id,
         'session-created', NEW.provider, NULL, NEW.created_at);
END;
...
BEGIN
    ...
    VALUES
        (NEW.user_id, NEW.device_id, NEW.client_id, NEW.session_id,
         'session-activated', NEW.provider, NEW.selected_route,
...
FOR EACH ROW
WHEN NEW.selected_route IS NOT OLD.selected_route AND NEW.selected_route IS NOT NULL
BEGIN
    INSERT INTO remote_computer_audit_events
        (user_id, device_id, client_id, session_id, event_type, provider, route, created_at)
    VALUES
        (NEW.user_id, NEW.device_id, NEW.client_id, NEW.session_id,
         'route-selected', NEW.provider, NEW.selected_route,
         CAST(strftime('%s','now') AS INTEGER));
END;
...
BEGIN
    UPDATE remote_computer_session_grants
       SET revoked_at = COALESCE(revoked_at, COALESCE(NEW.closed_at, CAST(strftime('%s','now') AS INTEGER)))
    ...
    VALUES
        (NEW.user_id, NEW.device_id, NEW.client_id, NEW.session_id,
         'session-closed', NEW.provider, NEW.selected_route,
         COALESCE(NEW.closed_at, CAST(strftime('%s','now') AS INTEGER)));

    INSERT INTO remote_computer_audit_events
        (user_id, device_id, client_id, session_id, event_type, provider, route, created_at)
    VALUES
        (NEW.user_id, NEW.device_id, NEW.client_id, NEW.session_id,
         'grant-revoked', NEW.provider, NEW.selected_route,
         COALESCE(NEW.closed_at, CAST(strftime('%s','now') AS INTEGER)));
END;

-- A session grant can only become more restrictive after creation. This fail-closed
-- guard prevents any provider/client path from escalating optional permissions by SQL.
CREATE TRIGGER IF NOT EXISTS remote_computer_session_grant_no_escalation
BEFORE UPDATE ON remote_computer_session_grants
FOR EACH ROW
WHEN NEW.allow_display > OLD.allow_display
  OR NEW.allow_input > OLD.allow_input
  OR NEW.allow_clipboard > OLD.allow_clipboard
  OR NEW.allow_file_transfer > OLD.allow_file_transfer
  OR NEW.allow_audio > OLD.allow_audio
BEGIN
