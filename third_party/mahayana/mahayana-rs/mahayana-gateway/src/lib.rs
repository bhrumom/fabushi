//! Mahayana-owned gateway semantics shared by CLI, desktop/native bridges and
//! future WebSocket/API adapters.
//!
//! The gateway deliberately adapts the existing Mahayana runtime instead of
//! introducing a second agent kernel. Runtime events are normalized into an
//! ordered session/turn stream, stamped with monotonically increasing sequence
//! numbers, and retained in a bounded replay ring for reconnecting clients.
...
impl ReplayStore {
    ...
    pub fn push(&mut self, draft: GatewayEventDraft) -> GatewayEvent {
        ...
        let encoded_size = serde_json::to_vec(&event).map_or(0, |encoded| encoded.len());

        // Oversized frames still consume a sequence number so the reconnecting
        // client can detect that replay has a hole and refetch history.
    ...
    pub fn since(&self, session_id: &str, last_seen: u64) -> ReplaySlice {
        if let Some(session) = self.sessions.get(session_id) {
            return ReplaySlice {
                ...
                truncated: last_seen < session.evicted_through,
                events: session
                    .events
                    .iter()
        ...
        ReplaySlice {
            ...
            last_seen,
            latest_seq,
            truncated: last_seen < latest_seq,
            events: Vec::new(),
        }
    }
    ...
    fn ensure_session(&mut self, session_id: &str) {
        ...
        // If the same session returns in this process epoch, continue its
        // counter instead of resetting to 1. This is stricter than accepting a
        // silent sequence reset after cache eviction.
...
impl RuntimeProjection {
    ...
    pub fn forget_turn(&mut self, turn_id: &str) {
        self.sessions_by_turn.remove(turn_id);
    }

    pub fn session_for_turn(&self, turn_id: &str) -> Option<&str> {
...
mod tests {
    ...
    #[test]
    fn evicted_session_keeps_sequence_and_forces_authoritative_refetch() {
        let limits = ReplayLimits {
        ...
        let forgotten = replay.since("s1", 0);
        assert!(forgotten.truncated);
        assert_eq!(forgotten.latest_seq, 1);
        assert!(forgotten.events.is_empty());

        let resumed = replay.push(draft("s1", "t3", 3));
        assert_eq!(resumed.seq, 2);
        assert!(replay.since("s1", 0).truncated);
    }
    ...
    #[test]
    fn message_delta_can_register_turn_before_lifecycle_events() {
        let mut state = GatewayState::with_epoch(ReplayLimits::default(), "epoch-1");
